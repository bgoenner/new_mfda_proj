...
SIM_ARGS= --netlist $(DESIGN).v --sim_dir $(SIM_PROJ_DIR) --design $(DESIGN) --sim_config $(SIM_FILE) --length_file $(SCAD_DOWN_DIR)/base/$(DESIGN)_lengths.xlsx --cir_config $(SIM_CIR_CONFIG) --docker_container $(SIM_DOCKER_CONTAINER) --lib $(LIBRARY_CSV) --xyce_write_loc spiceFiles
#--sim_file $(SIM_PROJ_DIR)
